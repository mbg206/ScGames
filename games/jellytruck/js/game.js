...
window.jt = {
    ...
    InitData: function () {
        ...
        if (window.localStorage) window.localStorage.setItem("jellytruck", JSON.stringify(this._data));
    },
    LoadData: function () {
        if (window.localStorage)
            return JSON.parse(window.localStorage.getItem("jellytruck"));
        else
            return -1;
    },
    SaveData: function () {
        if (window.localStorage) {
            window.localStorage.setItem("jellytruck", JSON.stringify(this.game_data));
        }
    },
    GetTotalScore: function () {
        if (window.localStorage) {
            var data = JSON.parse(window.localStorage.getItem("jellytruck"));
            this.total_score = 0;
